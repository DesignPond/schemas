...
class DbProjet implements ProjetInterface {
    ...
    public function getByArchives($projets_id = null, $filters){
	    
        $projets = $this->projet->used()->with(array('user'=> function($query)
        {
            $query->orderBy('users.nom', 'ASC');

        },'categorie','theme','subtheme','tags'))->where('type','=','app')->where('status','=','archive');
        		
        $projets->join('users', 'projets.user_id', '=', 'users.id')->select('users.nom', 'projets.*');						
        
        // If there is a date in filter
        if(isset($filters['date']) && $filters['date'])
        {  
	        $years = array( \Carbon\Carbon::createFromDate($filters['date'], 01, 01), \Carbon\Carbon::createFromDate($filters['date'], 12, 31) );
			
			$projets->whereBetween('created_at', $years);  
        }	
        
        // If there is a rating in filter
        if(isset($filters['rating']) && $filters['rating'])
        {
			$projets->where('rating', '=', $filters['rating']);        
        }	
        
        // If there is a mention in filter
        if(isset($filters['mention']) && $filters['mention'])
        {
			$projets->where('mentions', 'LIKE', '%'.$filters['mention'].'%');        
        }	
        
        // If there is a name in filter
        if(isset($filters['name']) && $filters['name'])
        {
			$projets->where('users.nom', 'LIKE', $filters['name'].'%');        
        }	
              					
        // If there is some ids for selected tags						
        if($projets_id)
        {
	       $projets->whereIn('id',$projets_id);  
        }						        
				
                
		return $projets->orderBy('users.nom', 'ASC')->simplePaginate(20);

    }    
    public function getArchives(){
		
		return $this->projet->where('type','=','app')->where('status','=','archive')->get();
